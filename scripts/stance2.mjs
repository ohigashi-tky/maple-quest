...
const measure = (ck)=>page.evaluate((charKey)=>{
  const s=window.__PHASER_GAME__.scene.getScene('Game');
  s.progress.charKey=charKey;
  let kb=0,total=0;
  for(let i=0;i<400;i++){
    if(!s.boss) break;
    s.player.x = s.boss.x + 40;     // 敵の右側に固定
    s.player.body.setVelocity(0,0); // ★毎回リセット
    s.invulnUntil=0;
    s.hurtPlayer(50);
...
console.log('5次戦士(stance100%):', JSON.stringify(await measure('warrior')));
console.log('5次魔法使い(stance50%):', JSON.stringify(await measure('mage')));
