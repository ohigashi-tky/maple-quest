...
await page.evaluate(()=>{ const s=window.__PHASER_GAME__.scene.getScene('Game'); s.player.x = s.boss.x - 90; s.charState.mp = s.maxMp('thief'); s.skShadow({name:'シャドーパートナー',kind:'shadow',mult:0,hits:0,targets:4,durMs:30000,mp:7,cd:30000}); });
await new Promise(r=>setTimeout(r,400));
// クアドラプルスロー → ヒット直後にスクショ
await page.touchscreen.tap(S(332, vh-78).x, S(332, vh-78).y);
await new Promise(r=>setTimeout(r,620));
await page.screenshot({ path:'/tmp/dmg2.png' });
await browser.close();
